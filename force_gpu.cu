...
cuda_ptr<RESULT::DensityGPU<VecForce> > DensityPolicy::dev_force;
...
template
PS::S32 RetriveKernel<RESULT::Density> (const PS::S32 tag,
					const PS::S32 n_walk,
					const PS::S32 ni[],
					RESULT::Density * force[]);

template
PS::S32 RetriveKernel<RESULT::ForceDPD> (const PS::S32 tag,
					 const PS::S32 n_walk,
					 const PS::S32 ni[],
...
PS::S32 DispatchKernel(const PS::S32 tag,
  ...
  //allocate array
  if(!ij_disp_allocated) {
    ij_disp.allocate(N_WALK_LIMIT + 2);    
    ij_disp_allocated = true;
  }
  ...
  const PS::S32 ni_tot = Policy::CopyToBuffer(n_walk, epi, n_epi, epj, n_epj, ni_tot, nj_tot);
  
  for(PS::S32 i = ni_tot; i < ni_tot_reg; i++)
    Policy::dev_epi[i].id_walk = n_walk;

  Policy::dev_epi.host2dev(ni_tot_reg);
  Policy::dev_epj.host2dev(nj_tot);
  
  const PS::S32 nblocks = ni_tot_reg / N_THREAD_GPU;

  if( typeid(Policy) == typeid(DensityPolicy) ) {
    DensityKernel <<< nblocks, nthreads >>> (ij_disp, Policy::dev_epi, Policy::dev_epj, Policy::dev_force);
  } else if ( typeid(Policy) == typeid(ForcePolicy) ) {
    ForceKernel   <<< nblocks, nthreads >>> (ij_disp, Policy::dev_epi, Policy::dev_epj, Policy::dev_force, Parameter::time);
  }
...
PS::S32 RetriveKernel(const PS::S32 tag,
  ...
  for(int k = 0; k < n_walk; k++)
    ni_tot += ni[k];
  Policy::dev_force.dev2host(ni_tot);
