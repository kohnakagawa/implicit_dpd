...
class ConfigMaker {
  ...
  void MakeCylindSheet(const PS::S32 axis,
    ...
    PS::U32 idx = offset;
    const PS::U32 end_idx = offset + num;
    MakeLineForEachAxis(z_elem, d_the_out, 0.0, out_rad, -1.0, axis, end_idx, idx); //out
    MakeLineForEachAxis(z_elem, d_the_in, 0.0, in_rad, 1.0, axis, end_idx, idx);  //in
    
    const PS::U32 residue = end_idx - idx;
    if (residue > 0) {
      MakeLineForEachAxis(in_the_elem, d_the_in, 0.5, out_rad, -1.0, axis, end_idx, idx);
      if (end_idx != idx) {
	std::cerr << "Too many particles are specified.\n";
	std::cerr << __FILE__ << " " << __LINE__ << std::endl;
	std::exit(1);
